...
**Status update (2026-05-08):** Austin replied to the alignment email (`correspondence/Document/email_austin_response_horizon_pool_2026-05-08.md`). Two of three confirmation questions answered: **CS's = Horizon Connection Servers** (confirmed); **policy enforcement layer = network firewall** (confirmed; not NSX micro-seg, not host-level). **New fact:** the Horizon pool uses the firewall as its default gateway → all egress flows through one chokepoint at Austin's firewall. Implication: when Snowflake/Azure egress allowlist mechanics land (Rohn's open sub-task), the deployment surface is this single firewall — Austin adds the rules, no per-VM/per-route configuration needed. **Sean's cluster-side view still pending** — that's the third question. Layering hypothesis (Austin's pool-level egress chokepoint + Sean's cluster-side ingress controls layering rather than replacing) is consistent with Austin's response. Mirrored in `Phases_Critical_Path_Development_v1.5.md` § BLOCKER.4 + `security/Notes/vendor-access-isolation-plan_2026-05-06.md`.
...
- [ ] [Phase1] [security] **Sync with Austin on Horizon pool ↔ cluster-side design layering** — triggered by Rohn's email 2026-05-07 (`correspondence/Document/email_sean_austin_horizon_pool_alignment_2026-05-07.md`). Austin's pool-level policies (pool egress: DNS/AD/UAG/CS's only, enforced at the network firewall = pool's default gateway) constrain the Horizon pool itself. Sean's cluster-side work would constrain what the pool can reach into msb-pmc03. **Austin replied 2026-05-08** (`correspondence/Document/email_austin_response_horizon_pool_2026-05-08.md`): network firewall confirmed as enforcement layer, default-gateway-is-firewall = single chokepoint. Layering hypothesis stands. **Sean's cluster-side view still owed** — that closes this joint item.
