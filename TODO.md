...
- [ ] [Phase1] [security] **Sync with Austin on Horizon pool ↔ cluster-side design layering** — triggered by Rohn's email 2026-05-07 (`correspondence/Document/email_sean_austin_horizon_pool_alignment_2026-05-07.md`). Austin's pool-level policies (pool egress: DNS/AD/UAG/CS's only) constrain the Horizon pool itself. Sean's cluster-side work would constrain what the pool can reach into msb-pmc03. Confirm whether they layer (most likely) or whether Austin's policies change the cluster-side scope. Output should fold into CP v1.5 vendor-isolation gate language.
