...
- [ ] [calculators] [security] **Self-contain native HTMLs — remove Google Fonts dep** — per CLAUDE.md § HTML Document Conventions § Self-Contained HTML rule (added 2026-05-08), all HTML in this project must be self-contained. The pandoc-rendered HTMLs in `html_review/` are now self-contained via `--embed-resources`. **Remaining gap:** native HTML source files reference Google Fonts via `<link href="https://fonts.googleapis.com/...">`. Affected files: `calculators/Document/cluster_sizing_tool.html`, `dev_cluster_math_reference.html`, `dev_calculator_guide.html`, `dev_slider_guide.html`, `dev-cluster-storage-reference.html`, `development_spark_calculator.html`, `production_spark_calculator.html`, `prod_calculator_guide.html`, `spark_flows.html`, `spark_glossary.html`; `security/Document/compliance_frameworks_reference.html`. Two paths to fix: (a) **Remove the Google Fonts `<link>` entirely** — fonts fall back to the already-listed system fonts (Cascadia Code, Fira Mono, Consolas, system-ui); slight visual change but trivial fix. (b) **Inline the fonts as base64 data-URIs** — preserves exact visual; per-file size grows ~30-100 KB per font weight (potentially MB-scale increase per file). Recommend (a) unless the IBM Plex Mono / DM Sans fonts specifically are visually load-bearing for the calculators.
