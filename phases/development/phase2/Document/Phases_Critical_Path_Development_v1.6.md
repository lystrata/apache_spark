...
_Companion tracker: `phases/development/phase2/Document/SOW_timeline_status.md` (full week calendar and SOW phase mapping)._
...
- **Verification gate (fqdn-side, lifts 3a) — all closed 2026-05-11:**
  ...
  - [x] Document the final encryption posture for HIPAA audit trail — **post-fix audit log at `phases/development/phase2/Incoming/ceph_audit_msb-pmc03-01.corp.fqdn_2026-05-11_1842.log` shows scratch drives bare-mounted (no dm-crypt) confirming hardware-only encryption; OSD drives remain LUKS-encrypted via dm-crypt**
...
- **Context:** Remote Airflow host coordinates ETL job submission to Spark cluster. Spec: **6 vCPU / 24 GB RAM / 500 GB SSD**, RHEL 9.4. **Target node: `msb-pmc01-04`** — Intel Xeon Gold 6136 @ 3.00 GHz, 1 socket × 12 cores × 2 threads (24 logical CPUs), 130 GB RAM, single NUMA node, 6 Ceph OSDs. Cluster is fqdn-managed; Ksolves provisions services into it. See `phases/development/phase2/Document/MSB-PMC01_cluster_host_inventory.md` for full hardware inventory of all four nodes (msb-pmc01-01 through 04). **Topology change (2026-05-07):** Ansible no longer co-located on the Airflow VM (see P1.5 below) — Ansible runs directly on Proxmox dev nodes. The Airflow VM hosts Airflow + Nginx (install in scope; activation TBD by Ksolves).
...
- **Ksolves Actions:**
  ...
  - [ ] Calculate average amplification factor and update `phases/development/phase1/deliverables/dev_cluster_phase1_model.html` default
- **Expected Output:** Actual shuffle amplification factor (e.g., 5.2× instead of assumed 7×) feeds into Phase 1 concurrency SLA
- **Verification:** `phases/development/phase1/deliverables/dev_cluster_phase1_model.html` updated with measured value
...
- **Source:** Vendor lead (Ksolves), email forwarded 2026-05-06 (referenced in TODO.md). Corroborated by the "Operational Gate Process → Move CSV files off → Designated Storage Node (Archive)" step (#10) in `phases/development/phase2/reference_images/dev_data_flow_diagram_2026-05-06.png`.
...
- **Vendor Questions:** phases/development/phase1/vendor_comms/phase1_vendor_questions.txt
- **Companion CP — Okta Integration:** phases/development/phase2/Document/CP_Okta_v1.1.md
- **Companion — Remote Airflow Host Briefing:** phases/development/phase2/Document/MSB-PMC01_airflow_host_briefing_v1.1.md
- **Schedule Tracker:** phases/development/phase2/Document/SOW_timeline_status.md
