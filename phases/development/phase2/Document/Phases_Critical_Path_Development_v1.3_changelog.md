...
The v1.4 bump captures three substantive changes the project decided on 2026-05-05 in response to Ksolves' authoritative configuration document delivered 2026-05-04 (`phases/development/phase2/Document/Ksolves_Spark_YARN_Config_v1.0.pdf`):
...
2. **HIPAA scope forked into a separate critical-path sub-project.** Vendor's § 8 (HIPAA Compliance Architecture & Guidelines) introduces a substantial cross-team workstream — Kerberos / RPC encryption / SSE / a custom javax servlet filter for Web UI ACL. Tracking it in the main CP would obscure the cross-team coordination, so it lives at `phases/development/phase2/Document/CP_HIPAA_Compliance_v1.0.md` with a BLOCKER pointing back to it.
...
| New companion file | `phases/development/phase2/Document/CP_HIPAA_Compliance_v1.0.md` (and `.pdf`) — sub-project critical path for the HIPAA encryption + Web UI ACL workstream. Promoted from `Ready_For_Review/` to `Document/` as part of this v1.4 bump. |
...
**Cross-references _not_ bumped (deferred to a follow-up):** `phases/development/phase1/README.md`, `phases/development/phase1/Notes/ksolves-april24-status-review.md`, `phases/development/phase1/vendor_comms/phase1_vendor_questions.txt`, `phases/development/phase2/Document/CP_Okta_v1.0.md`, `phases/development/phase2/Document/MSB-PMC01_airflow_host_briefing_v1.0.md`, `phases/development/phase2/Document/SOW_timeline_status.md`, `calculators/Document/cluster_sizing_tool.html`, `calculators/Document/dev_cluster_math_reference.html`, `calculators/Document/dev-cluster-storage-reference.html`, `phases/development/phase{1,2}/deliverables/dev_cluster_phase1_model.html`. These will be updated in subsequent commits as needed.

**Not changed:**
- `phases/development/phase2/Document/Phases_Critical_Path_Development_v1.3.md` (the now-archival v1.3) is left as-is — it accurately represents the state at the v1.3 promotion.
- `phases/development/phase2/Document/Phases_Critical_Path_Development_v1.2.md` and `_v1.1_changelog.md`, `_v1.2_changelog.md` remain as historical records.
- `phases/development/phase2/Document/phases_critical_path_development_tracker_v1.3.html` left intact alongside the new v1.4 tracker (archival reference).
