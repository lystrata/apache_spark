...
_Report Source: phases/development/phase1/Incoming/fqdn Report Phase 1 (Updated).docx.pdf (development scope; production scope to be defined separately)_  
...
- **Open Vendor Question:** ZooKeeper placement — host-level deployment on each Proxmox node, or small dedicated ZK VM on each node? Tracked in `phases/development/phase1/vendor_comms/phase1_vendor_questions.txt` § Vendor Daily Meeting (2026-04-23). Confirm with Ksolves before installation begins. Default assumption: host-level (lowest overhead, fewer moving parts).
